...
func (n *node) handleMessageAlice(who peer.ID, msg net.Message, setupDone chan struct{}) error {
		...
		go func() {
			for {
				...
				case kp := <-claim:
					...
					if err != nil {
						log.Debug("failed to create judecoin address: %s", err)
						return
					}

					log.Info("successfully created judecoin wallet from our secrets: address=", address)
	...
	case *net.NotifyClaimed:
		...
		if err != nil {
			log.Error("failed to create judecoin address: err=", err)
			return err
		}
