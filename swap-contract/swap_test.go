...
func TestSwap_Claim(t *testing.T) {
	// Alice generates key
	keyPairAlice, err := judecoin.GenerateKeysTruncated()
	require.NoError(t, err)
	// pubKeyAlice := keyPairAlice.PublicKeyPair().SpendKey().Bytes()
	pubKeyAliceX, pubKeyAliceY := judecoin.PublicSpendOnSecp256k1(keyPairAlice.SpendKeyBytes())
	...
	// Bob generates key
	keyPairBob, err := judecoin.GenerateKeysTruncated()
	require.NoError(t, err)
	secretBob := keyPairBob.SpendKeyBytes()
	pubKeyBobX, pubKeyBobY := judecoin.PublicSpendOnSecp256k1(secretBob)
